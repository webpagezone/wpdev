	...
	<div class="col-lg-7">
		...
		<div class="CWproduct CWcontent" id="CWdetails">
			...
			<cfif product.product_on_web eq 'true' and product.product_archive neq 1
				...
				<h2>#product.product_name#</h2>
					...
					<div id="CWproductInfo">
						...
						<form action="#formActionUrl#" class="form-inline CWvalidate" method="post" name="AddToCart" id="CWformAddToCart">
							...
							<cfif len(trim(product.product_custom_info_label)) AND application.cw.adminProductCustomInfoEnabled>
								<!--- custom value --->
									...
									<div class="row CWcustomInfo">

											<label class="wide col-lg-4 text-right" for="customInfo">#trim(product.product_custom_info_label)#:</label>

										<div class="col-lg-8">
											<textarea name="customInfo" class="form-control custom"   maxlength="255">#form.customInfo#</textarea>
											<!--- <input type="text" name="customInfo" class="form-control custom" size="22" value="#form.customInfo#" maxlength="255"> --->
										</div>
									</div>
							...
							<cfif request.cwpage.stockok eq true>
											...
											<div id="CWproductPrices">
												...
												<cfif request.cwpage.useAltPrice eq 'true'>
													<div class="CWproductPriceAlt">
														<label >#request.cwpage.altPriceLabel#:</label >
								...
								<!--- if stock is not ok --->
							<cfelse>
								<div class="CWalertBox alertText alert alert-dismissible alert-info">#product.product_out_of_stock_message#</div>
							</cfif>

							<!--- confirmation / alerts --->
							<cfif len(trim(request.cwpage.cartconfirm))>
								<!--- list confirmations, add view cart link --->
								<div class="CWconfirmBox confirmText alert alert-dismissible alert-info"><div>#replace(request.cwpage.cartconfirm,',','</div><div>','all')#</div><a href="#request.cwpage.urlShowCart#">View Cart</a></div>
								<cfset structDelete(session.cw,'cartConfirm')>
							</cfif>
							<cfif len(trim(request.cwpage.cartAlert))>
								<div class="CWalertBox alertText fadeOut alert alert-dismissible alert-danger"  style="border:1px solid ##ccc;"><div>#replace(request.cwpage.cartAlert,',','</div><div>','all')#</div></div>
								<cfset structDelete(session.cw,'cartAlert')>
							</cfif>
				...
				<cfif len(trim(product.product_description))>
					<div class="CWproductDescription small">#product.product_description#</div>
					<!--- continue shopping --->
			...
			<cfif len(trim(product.product_special_description))>
				<div class="CWproductSpecialDescription small">#product.product_special_description#</div>
			</cfif>
