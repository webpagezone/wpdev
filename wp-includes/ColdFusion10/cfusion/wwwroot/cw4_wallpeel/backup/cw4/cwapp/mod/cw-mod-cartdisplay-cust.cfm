...
<!--- HANDLE FORM SUBMISSION --->
	...
	<p class="CWcartTotals">
		...
		<cfif attributes.show_discount_total and application.cw.discountsEnabled and cwcart.carttotals.cartDiscounts gt 0>
			<span >Item Total:</span><span class="CWtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.sub + cwcart.carttotals.cartDiscounts,'local')#</cfoutput> </span><br>
			<span class="CWdiscountText">Discounts:</span><span class="CWtotalText CWdiscountText">-<cfoutput>#lsCurrencyFormat(cwcart.carttotals.cartDiscounts,'local')#</cfoutput> </span><br>
		</cfif>
		<!--- cart subtotal --->
		<span class="CWsubtotalText">Subtotal:</span><span class="CWtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.sub,'local')#</cfoutput> </span>
		<!--- edit cart link --->
		...
		<cfif attributes.show_tax_total>
			<span ><cfoutput>#application.cw.taxSystemLabel#</cfoutput>:</span><span class="CWtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.tax,'local')#</cfoutput> </span><br>
		</cfif>
		...
		<cfif attributes.show_ship_total>
			<cfif application.cw.shipEnabled
				...
				<!--- shipping base cost --->
				<span >Shipping/Handling:</span><span class="CWtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.shipping + cwcart.carttotals.shipDiscounts,'local')#</cfoutput> </span>
				<!--- shipping discounts --->
				<cfif application.cw.discountsEnabled and cwcart.carttotals.shipDiscounts gt 0 and attributes.show_discount_total>
					<br><span class="CWdiscountText">Shipping Discounts:</span><span class="CWdiscountText CWtotalText">-<cfoutput>#lsCurrencyFormat(cwcart.carttotals.shipDiscounts,'local')#</cfoutput> </span>
					<br><span class="CWsubtotalText">Shipping Total:</span><span class="CWsubtotalText CWtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.shipping,'local')#</cfoutput> </span>
				</cfif>
				<br>
				<!--- shipping value message (if error or other text returned for ship total above) --->
				<cfif isDefined('request.cwpage.shipRateError')>
					<span >&nbsp;</span><span class="CWtotalText"><cfoutput>#request.cwpage.shipRateError#</cfoutput> </span><br>
				</cfif>
				<!--- shipping tax --->
				<cfif isDefined('cwcart.carttotals.shippingTax')
					...
					<cfoutput>
						<span >Shipping #application.cw.taxSystemLabel#:</span><span class="CWtotalText">#lsCurrencyFormat(cwcart.carttotals.shippingTax,'local')# </span><br>
					</cfoutput>
				<cfelse>
	...
	<!--- BALANCE / PAYMENT TOTALS --->
			...
			<cfif attributes.show_order_total>
				<p class="CWtotal">
				<span class="CWsubtotalText">Order Total:</span><span class="CWtotalText CWsubtotalText"><cfoutput>#lsCurrencyFormat(cwcart.carttotals.total,'local')#</cfoutput> </span>
				</p>
				<!--- show payments total --->
				<cfif orderPayments gt 0 and attributes.show_payment_total>
					<p class="CWtotal">
					<span class="CWsubtotalText">Payments Made:</span><span class="CWtotalText CWsubtotalText">-<cfoutput>#lsCurrencyFormat(orderPayments,'local')#</cfoutput> </span>
					</p>
					<!--- balance due --->
					<p class="CWtotal">
					<span class="CWsubtotalText">Balance Due:</span><span class="CWtotalText CWsubtotalText"><cfoutput>#lsCurrencyFormat(tempTotal,'local')#</cfoutput> </span>
					</p>
				</cfif>
			</cfif>
	...
	<cfelse>
			...
			<cfif  attributes.display_mode eq 'showcart'>
			<form name="updatecart" action="<cfoutput>#attributes.form_action#</cfoutput>" method="post"
			id="formUpdateCart" class="form-inline">
			</cfif>
				<!--- CART PRODUCTS TABLE --->
				<!--- products in cart--->
				<table class="table table-striped table-bordered" id="CWcartProductsTable">
					<thead>
						<!--- alert row --->
							...
							<td colspan="<cfoutput>#request.cwpage.cartColumnCount#</cfoutput>" class="noPad">
								<cfif isDefined('request.cwpage.cartAlert') and len(trim(request.cwpage.cartAlert))>
									<cfoutput>
										...
										<div class="CWalertBox alertText alert alert-dismissable alert-danger">
								...
								<cfif isDefined('request.cwpage.cartconfirm') and len(trim(request.cwpage.cartconfirm))>
									<cfoutput>
										...
										<div class="CWconfirmBox confirmText alert alert-success">
										 <button type="button" class="close" data-dismiss="alert">×</button>
						...
						<tr class="headerRow">
							...
							<cfif  attributes.display_mode eq 'showcart'>
							<th class="center notBold smallPrint"><input type="checkbox"class="form-control" rel="checkAllRemove" name="checkAllProducts" class="checkAll" tabindex="1"> Remove</th>
							</cfif>
						</tr>
					</thead>
					<tbody>
								...
								<tr class="#rowClass#">
									...
									<td class="productCell">
										...
										<cfif len(trim(CWcartItem.cartImg))>
											<div class="CWcartImage img-border">
														...
														<cfif application.cw.appDisplayCartCustomEdit and attributes.display_mode eq 'showcart'>
															<span class="CWcartCustomInfo">
																...
																<span class="CWcartChangeInput">
																	<input type="text" name="customInfo#cartLine#" class="form-control" size="22" value="#phraseText#" maxlength="255">
																</span>
															</span>
														</cfif>
													</div>
									...
									<td class="qtyCell center" >
										...
										<input name="qty#cartLine#" style="width:60px;"
										type="number"  min="1" required
										value="#CWcartItem.quantity#"
										size="2"
									...
									<td class="checkboxCell center">
										...
										class="formCheckbox checkAllRemove form-control" value="#CWcartItem.SkuUniqueID#" rel="group1">
									</td>
									</cfif>
								</tr>
							</cfloop>
							...
							<td>
								...
								<cfif attributes.show_discount_descriptions>
									...
									<cfif listLen(request.cwpage.discountDescriptions,'|')>
										...
										<p class="CWdiscountHeader"><strong>Discounts applied to this order:</strong></p>
							...
							<td colspan="<cfoutput>#request.cwpage.cartColumnCount - 3#</cfoutput>" class="CWright totalCell">
								<cfif attributes.show_total_row>
									...
									<cfif attributes.show_discount_total
										...
										<span class="CWdiscountText">Discounts: </span><br>
									</cfif>
									<!--- subtotal label --->
									<span class="CWsubtotalText">Subtotal: </span>
									<!--- tax label --->
									<cfif attributes.show_tax_total>
									<br><span ><cfoutput>#application.cw.taxSystemLabel#</cfoutput>: </span>
									</cfif>
									...
									<cfif application.cw.shipEnabled
										...
										<cfif attributes.show_ship_total>
											<br><span >Shipping/Handling: </span>
											<!--- shipping discounts --->
											<cfif application.cw.discountsEnabled
												and cwcart.carttotals.shipDiscounts gt 0
												and attributes.show_discount_total>
											<br><span class="CWdiscountText">Shipping Discounts: </span>
											<br><span class="CWsubtotalText">Shipping Total: </span>
											</cfif>
											<!--- shipping tax --->
											<cfif isDefined('cwcart.carttotals.shippingTax') and cwcart.carttotals.shippingTax gt 0 and request.cwpage.taxChargeOnShipping>
												<br><span >Shipping <cfoutput>#application.cw.taxSystemLabel#</cfoutput>: </span>
											</cfif>
										</cfif>
										<!--- order total label --->
										<cfif attributes.show_order_total>
											<br><span class="CWsubtotalText">Order Total: </span>
										</cfif>
									</cfif>
									<!--- payment total --->
									<cfif orderPayments gt 0 and attributes.show_payment_total>
									<br><span class="CWsubtotalText">Payments Made: </span>
									</cfif>
								</cfif>
							</td>
							<!--- total amounts --->
						...
						<td class="center">
							<!--- update button --->
							<input name="updateCart"  type="submit" class="CWformButtonSmall btn btn-primary" id="update" value="Update">
	...
	<div class="CWpromoCode">
		<form name="cartpromo" action="<cfoutput>#attributes.form_action#</cfoutput>" method="post" id="formCartPromo">
			...
			<input name="submitPromo" type="submit" class="CWformButtonSmall btn " id="CWsubmitPromo" value="Apply Code">
		</form>
