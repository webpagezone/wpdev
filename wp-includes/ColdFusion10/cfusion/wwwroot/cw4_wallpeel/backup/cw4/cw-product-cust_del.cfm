...
<cfset product.productImgZoom = CWgetImage(product_id=request.cwpage.productID,image_type=3,default_image=application.cw.appImageDefault)>
<!--- make sure file exists for zoom link (avoid broken links) --->
<cfif NOT (len(trim(product.productImgZoom)) AND fileExists(expandPath(product.productImgZoom)))>
				...
				<cfoutput>
					...
					<div class="product-div-img ">
						<cfif request.cwpage.imageZoom>
							...
							<a href="#product.productImgZoom#" class="CWimageZoomLink CWlink" title="#product.product_name#"><img src="#product.productImgMain#" alt="#product.product_name#: click to enlarge"></a>
							<div class="clicktoEnlarge">
								<a href="#product.productImgZoom#" class="CWimageZoomLink CWlink" title="#product.product_name#">Click to Enlarge</a>
							</div>
						<cfelse>
							<!--- show only the image, no link, no zoom --->
							<img src="#product.productImgMain#" alt="#product.product_name#">
						</cfif>
					</div>
					<!-- /end product image -->
					</cfoutput>
		...
		<!-- product name -->
				<h2>#product.product_name#</h2>
					...
					<div id="CWproductInfo">
						...
						<form action="#formActionUrl#" class="form-inline"
						 method="post" name="AddToCart" id="CWformAddToCart">
							 <fieldset>


							<!-- product options: shows select lists or table based on variable setting for 'display type' -->
							<cfmodule
							template="cwapp/mod/cw-mod-productoptions-cust.cfm"
...
<!--- 						<p id="quantity_wanted_p">
							...
							<!--- dropdowns only (or table with no options): tables method includes its own quantity fields --->
								<cfif request.cwpage.optionDisplayType neq 'table' or len(trim(product.optiontype_ids)) lt 1>
									...
									<cfif application.cw.appDisplayProductQtyType is 'text'>
										...
										<!--- <input name="qty" style_="width:40px; padding: 0px; margin:0;height:40px;"
										id="qtyInput"
										value="<cfoutput>#request.cwpage.intQty#</cfoutput>"
										class="{required:true,number:true,min:1} qty"
										title="Quantity is required" size="2"
										onkeyup="extractNumeric(this,0,false)"
										type="number" min="1" required
										> --->

										<input name="qty" style="width:50px;"
										id="qtyInput"
										value="<cfoutput>#request.cwpage.intQty#</cfoutput>"
						...
						<div class="col-lg-4">
							...
							<div id="CWproductPrices">
								<div class="CWproductPrice<cfif request.cwpage.hasDiscount eq 'true'> strike</cfif>">
									<label >Price:</label>
									<span class="badge">#lsCurrencyFormat(product.price_low,'local')#</span>
							...
							<cfif request.cwpage.stockok eq true>
								...
								<div>
									...
									<div class="center CWclear">
										...
										class="btn btn-primary"  title="Add to Cart">
						...
						<div class="row">
							<div class="col-lg-12">
								<cfelse>
								<div class="CWalertBox alertText alert alert-dismissable alert-danger">#product.product_out_of_stock_message#</div>
								</cfif>
								<!--- confirmation / alerts --->
								...
								<cfif len(trim(request.cwpage.cartAlert))>
									<div class="CWalertBox alertText fadeOut alert alert-dismissable alert-danger"><div>#replace(request.cwpage.cartAlert,',','</div><div>','all')#</div></div>
									<cfset structDelete(session.cw,'cartAlert')>
								</cfif>
							<!--- hidden values --->
		...
		<!-- product description -->
						<cfif len(trim(product.product_description))>
							...
							<cfif isDefined('request.cwpage.returnUrl') and len(trim(request.cwpage.returnUrl))>
								<p class="clicktoEnlarge">&raquo;&nbsp;<a href="<cfoutput>#request.cwpage.returnUrl#</cfoutput>">Continue Shopping</a></p>
							</cfif>
						</cfif>
...
<!-- related products -->
			...
			<cfif len(trim(product.related_product_ids))>
				<!--- show all related products in this area --->
					<h4 class="well well-sm">Related Items:</h4>
...
<cfinclude template="cwapp/inc/cw-inc-recentview-cust.cfm">
