...
$( document ).ready(function() {
	...
	var _bdesignGroupLeft;
	var _bdesignGroupTop;	
	...
	$('#loadBackDesign').click(function(){
		if($('#designName').val() !== ""){
			$(".canvas-container:first").hide();
			$('#canvasBack').css('opacity',0);
			$('#saveFrontDesign').hide();
			$('#saveBackDesign').hide();			
			$('#HQblockArea').fadeIn();
			_currentName = $('#designName').val();
			var _curBackDesignURL = '../orders/'+_currentName+'/'+_currentName+'_back.json';    
			$('#designName').removeClass('error');
			$.ajax({
	            type : 'POST',
	            url : 'checkFiles/checkBackDesign',     
	            data: {
	                ...
	                if(data === 'true'){
	                	$.ajax({
						    ...
						    success: function(xml) {
						        $(xml).find('info').each(function(e, i)
						        {
						            _bdesignGroupLeft = $(i).attr("bdesignLeft");
						            _bdesignGroupTop = $(i).attr("bdesignTop");  
						            _bHQLeft = Math.floor(_HQCanvasWidth*_bdesignGroupLeft/600);
						            _bHQTop = Math.floor(_HQCanvasHeight*_bdesignGroupTop/600);				            								
								    $.ajax({
								        dataType: 'json',
								        success: function(data) {
								        	if(data.objects.length>2){
								        		_designCanvasBack.loadFromJSON(data);
								           		$('#saveBackDesign').show();
								           		$('#alert').text('');
								           		setTimeout(function() { loadbHQDesign(); }, 5000);
								        	}else{
								        		$('#alert').text('No Back Design available!');
								        		$('#HQblockArea').hide();
								        	}
								    	},
								        url: './orders/'+_currentName+'/'+_currentName+'_back.json'
								    })
						        })
						    }    
						})
	...
	function loadbHQDesign(){
		_designCanvasBack.setActiveGroup(new fabric.Group(_designCanvasBack.getObjects(),{scaleX:_scale, scaleY:_scale, left:_bHQLeft, top:_bHQTop})).renderAll();
		_designCanvasBack.forEachObject(function(o) {
		  o.selectable = false;
		});
		_designCanvasBack.deactivateAll().renderAll();
		_designCanvasBack.calcOffset();
		$('#canvasBack').css('opacity',1);
		$('#HQblockArea').fadeOut();
	}
...
<div id="loadFrontDesign" class="elementHolder orange">Load Front HQ Design</div>
<div id="loadBackDesign" class="elementHolder orange">Load Back HQ Design</div>
<div id="saveFrontDesign" class="elementHolder green hidden">Save Front HQ Design</div>
<div id="saveBackDesign" class="elementHolder green hidden">Save Back HQ Design</div>
<div id="alert"></div>
</div>

<div id="HQDesign">
	<canvas id="canvasFront" width="1600" height="1600" style='border:1px solid #ccc; opacity:0;'></canvas>
	<canvas id="canvasBack" width="1600" height="1600" style='border:1px solid #ccc; opacity:0;'></canvas>
</div>	
</body>
</html>