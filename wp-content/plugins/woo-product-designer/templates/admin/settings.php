...
<h2>
	<?php printf(__("%s", 'nm-wcpd'), $this->plugin_meta['name']);?>
</h2>
<div id="nm_wcpd-tabs" class="wrap">
	<ul class='etabs'>
	...
	<div id="<?php echo $id?>">
		...
		<?php foreach($options['meat'] as $key => $data){
			...
			?>
				<tr valign="top">
					<th scope="row" width="25%"><?php printf(__('%s', 'nm-wcpd'), $label);?>
	...
	?>
	<p class="the-button"><button class="button button-primary"><?php _e('Save settings', 'nm-wcpd')?></button><span class="nm-saving-settings"></span></p>
	</form>
