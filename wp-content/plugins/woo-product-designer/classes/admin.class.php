...
class NM_ProductDesigner_Admin extends NM_ProductDesigner{
	...
	function __construct(){
		...
		*/
		...
		//adding meta box into products edit
		add_action ( 'admin_init', array (
				$this,
				'add_woodesign_meta_box' 
		) );
		...
		foreach ( $this->plugin_scripts_admin as $script ) {
			...
			if( $attach_script ){
				...
				$arrLocalizedVars = array (
						...
						'plugin_admin_page' => admin_url ( 'options-general.php?page=nm_wcpd' )
				);
	...
	*/
	...
	function add_menu_pages(){
		...
		foreach ($this -> menu_pages as $page){
			...
			if ($page['parent_slug'] == ''){
				...
				$menu = add_menu_page(__('Product Designer', 'nm-wcpd'),
						__('Product Designer', 'nm-wcpd'),
						$page['cap'],
						$page['slug'],
				...
				$menu = add_submenu_page($page['parent_slug'],
						__($page['page_title'].' Settings', 'nm-wcpd'),
						__($page['menu_title'].' Settings', 'nm-wcpd'),
						$page['cap'],
						$page['slug'],
						array($this, $page['callback'])
				);
	...
	function save_settings(){
		...
		_e('All options are updated', 'nm-wcpd');
		die(0);
	}
	...
	function render_settings_input($data) {
		...
		if(isset($this ->plugin_settings[ $data['id']]) && !is_array($this ->plugin_settings[ $data['id']]))
			$value		=  (isset($this ->plugin_settings[ $data['id']])) ? stripslashes( $this ->plugin_settings[ $data['id']] ) : '' ;
		else 
		...
		switch($type) {
			...
			case 'media' :
				...
				echo ' <a href="javascsript:;" class="remove-media"><i class="fa fa-pencil"></i>'.__('Remove', 'nm-wcpd').'</a>';
	...
	function add_woodesign_meta_box() {
		add_meta_box ( 'woodesigns_order', __('Product Design','nm-wcpd'),
					array ($this, 'display_woodesigns'), 
					'shop_order', 'normal', 'default' );
					
	}
	
	function display_woodesigns($order) {

		$woodesign_metas = get_post_meta ( $order->ID, 'canvas_meta', true );
		
		
		$template_vars = array('woodesign_metas' => $woodesign_metas);
		$this -> load_template('admin/admin-canvas.php', $template_vars);
